<?php
// update_db.php — УСТАРЕВШИЙ И ОПАСНЫЙ СКРИПТ
// ВНИМАНИЕ: Этот скрипт переименовывает таблицу 'marketplaces' -> 'case_themes'
// и поле 'marketplace_id' -> 'theme_id', что СЛОМАЕТ установку, работающую
// со стандартной схемой из db.sql. Проект использует схему из db.sql (marketplaces + marketplace_id).
// Не запускайте этот скрипт. Используйте db.sql для инициализации базы данных.
exit('Этот скрипт отключён в целях безопасности. Схема БД управляется через db.sql. Запуск запрещён.');
